...
BOT_TOKEN = environ.get('BOT_TOKEN', "7126708211:AAEn4R1ktcDeqNecaSnM-jjbQ9M8QcIgv7E")
...
ADMINS = [int(admin) if id_pattern.search(admin) else admin for admin in environ.get('ADMINS', '6941610925 6246830040').split()]
CHANNELS = [int(ch) if id_pattern.search(ch) else ch for ch in environ.get('CHANNELS', '-1002075903430').split()] #Channel id for auto indexing ( make sure bot is admin )
auth_users = [int(user) if id_pattern.search(user) else user for user in environ.get('AUTH_USERS', '').split()]
AUTH_USERS = (auth_users + ADMINS) if auth_users else []
PREMIUM_USER = [int(user) if id_pattern.search(user) else user for user in environ.get('PREMIUM_USER', '6941610925').split()]
auth_channel = environ.get('AUTH_CHANNEL', '-1002021217229') #Channel / Group Id for force sub ( make sure bot is admin )
...
reqst_channel = environ.get('REQST_CHANNEL_ID', '-1002160257812') # request channel id ( make sure bot is admin )
